          ...
          <tr v-for="(donDatPhong, index) in donDatPhongs" :key="index">
            ...
            >
              <thanh-toan-button
                ...
                @getInfor="getInfor(donDatPhong.id)"
              ></thanh-toan-button>
            </td>
          </tr>
        </tbody>
...
import ThanhToanButton from "../Buttons/ThanhToanButton.vue";
...
export default {
  ...
  methods: {
    ...mapActions("donDatPhong", ["thanhToan", "searchDonDatPhongs", "getDonDatPhongByStatus"]),
    getInfor(id) {
      const isThanhToan = confirm("Bạn có chắc chắc chắn thanh toán?");
      if (isThanhToan) {
        this.thanhToan({ id });
        window.location.assign("/employee/thanh-toan");
      }
    },
    async searchDDP() {
      const search = {
      ...
      if(search.key === undefined || search.key === '') {
        await this.getDonDatPhongByStatus();
      } else {
